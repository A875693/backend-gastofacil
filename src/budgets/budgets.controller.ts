...
export class BudgetsController {
   ...
   */
  @Get('daily-allowance')
  @ApiOperation({ summary: 'Calcular cuánto puede gastar hoy el usuario' })
  @ApiResponse({ 
    status: 200, 
    description: 'Cálculo de gasto diario permitido',
    type: DailyAllowanceDto 
  })
  @ApiResponse({ status: 400, description: 'No hay presupuesto configurado' })
  getDailyAllowance(@GetUser() user: DecodedIdToken): Promise<DailyAllowanceDto> {
    return this.budgetsService.getDailyAllowance(user.uid);
  }
